...
class Queen:
    ...
    def can_move(self, board, row, col, row1, col1):
        if board.field[row][col] is not self or not correct_coords(
                row1,
                col1) or board.field[row1][col1] is not None and board.field[
                    row1][col1].color() == self.color():
            return False
        if abs(row - row1) == abs(col - col1):
            for i in range(min((row, row1)) + 1, max((row, row1))):
                for j in range(min((col, col1)) + 1, max((col, col1))):
